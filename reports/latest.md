...
- 발생 빈도: 106건 (최근 3개월 80건 / 이전 9개월 26건)
- 증가율: +823%
- 대표 VOC: "samsung has really gone downhill from losing the samsung message app too google and now this not functioning properly. my television can be on and on the same network but this app still says it's off and offline. samsung…" — play, 2026-07-27 ([원문](https://play.google.com/store/apps/details?id=com.samsung.android.oneconnect))
- Pain Point 요약: 최근 3개월 발생 빈도가 이전 대비 +823% 증가하며, 'Connectivity & Reliability' 관련 부정 VOC가 총 106건 누적되어 상위 Pain Point로 식별됨.
...
- 증가율: 신규
- 대표 VOC: "Horrible app. I installed this to help the connection process, upon doing so I go through and put in all of my information, but when it sends the verification code, one of two things happen; I get no notification that th…" — play, 2026-07-22 ([원문](https://play.google.com/store/apps/details?id=com.samsung.android.oneconnect))
- Pain Point 요약: 최근 3개월 내 새롭게 부각된 이슈로, 'Notification & Alert' 관련 부정 VOC가 총 1건 누적되어 상위 Pain Point로 식별됨.
...
Apple Home, Google Home, Aqara, Home Assistant 등 경쟁/대체 플랫폼과 비교하며 SmartThings의 열위를 지적한 VOC 5건 확인. 주로 **Multi-brand Integration** 카테고리에서 함께 언급됨.
...
- "Hub Firmware and Edge Driver Release Notes - 0.61.6 ...over the place. I hate to be that guy, but for a few months now, as devices drop off SmartThings and I ca…" — smartthings_community, 2026-06-25 ([원문](https://community.smartthings.com/t/hub-firmware-and-edge-driver-release-notes-0-61-6/309818/44))

### 차기 릴리즈 우선 검토 항목

증가율 기준 상위 카테고리를 차기 릴리즈 검토 우선순위로 제안합니다:

1. **AI / Voice** (신규) — AI/음성 기능이 특정 지역·기기에서만 제공되어 기대와 실제 지원 범위 간 괴리가 크고, Bixby 등 음성 인식 정확도가 낮아 명령 실패가 반복됨.
2. **Notification & Alert** (신규) — 푸시 알림 파이프라인(클라우드→FCM/APNs)의 지연, 사용자별 알림 설정과 기기 이벤트 트리거 간 매핑 오류로 알림 누락/중복 발생.
3. **Connectivity & Reliability** (+823%) — 허브-클라우드 간 세션 유지 로직 취약, 다중 홉 메시 네트워크에서의 신호 간섭, 백그라운드 재연결 로직 미흡으로 오프라인 표시가 실제 상태와 어긋남.
